    ...
    dhcp_options_t *options)
{
    ...
    /* setup lease timer */
    lease_duration = dtime_from_seci(options->lease_time);
    ...
    /* only for debuggung */
        ...
        /* show to the world! */
        dprintf_i("got valid lease for ip: %s, subnet: %s, gw: %s\n",
...
static err_t TCPIPDhcp_Input(void *arg, size_t size)
{
    ...
    /* dhcp offer */
    case TCPIP_DHCP_MSG_TYPE_OFFER:
        return TCPIPDhcp_ProcessOffer(dhcp, &options);
    /* dhcp acknowledgment */
    case TCPIP_DHCP_MSG_TYPE_ACK:
        return TCPIPDhcp_ProcessAck(dhcp, &options);
    /* no ack message */
    case TCPIP_DHCP_MSG_TYPE_NAK:
        return TCPIPDhcp_ProcessNak(dhcp, &options);
    /* unsupported message */
    default:
        return EFATAL;
    }
...
err_t TCPIPDhcp_Init(void)
{
    ...
    if (Yield_Task(TCPIPDhcp_TxTask, 0, 2048) != EOK)
        return EFATAL;
    /* create state machine task */
    if (Yield_Task(TCPIPDhcp_RxTask, 0, 2048) != EOK)
        return EFATAL;
