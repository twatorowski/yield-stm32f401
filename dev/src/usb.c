...
/* in endpoints */
static usb_epin_t ep_in[6];
/* out/control endpoints */
static usb_epout_t ep_out[6];
...
static void USB_HandleReset(void)
{
    ...
    for (usb_epin_t *in = ep_in; in != ep_in + elems(ep_in); in++)
        in->ec = EFATAL;

    /* set out endpoint in error state */
    for (usb_epout_t *out = ep_out; out != ep_out + elems(ep_out); out++)
        out->ec = EFATAL;
...
//     usb_epout_t *out = ep_out; usb_oe_t *oe;
...
/* output endpoint interrupt */
static void USB_HandleOUTTransfer1(void)
{
    /* out endpoint pointer & register bank */
    usb_epout_t *out = ep_out; usb_oe_t *oe;
...
static void USB_HandleINTransfer(void)
{
    ...
    usb_epin_t *in = ep_in; usb_ie_t *ie;
    ...
    dtime_t timeout)
{
    ...
    in->ptr = ptr, in->size = size, in->offs = 0; in->ec = EBUSY;
